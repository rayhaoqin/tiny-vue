...
export const computedIsLast = ({ state, props }) => () =>
  state.index === props.urlList.length - 1

export const computedCurrentImg = ({ state, props }) => () =>
...
export const handleVisible = ({ state, emit, props }) => () => {
  ...
  setTimeout(() => {
    if (props.startPosition !== 0) {
      state.index =
        (props.startPosition - 1 + state.urlList.length) % state.urlList.length
...
export const getImageWidth = ({ state, parent, props }) => () => {
  ...
  imageW = parent.$el.querySelector('.tiny-mobile-image-viewer__canvas')
    .offsetWidth

  state.imageList = parent.$el.querySelectorAll(
    '.tiny-mobile-image-viewer__item'
  )
...
export const swipeLeft = ({ state, emit }) => () => {
  ...
  if (
    state.imageTransform == state.iamgeAllWidth - state.imageItemWidth &&
...
export const touchmove = (state) => (event) => {
  ...
  if (events2) {
    ...
    const getDistance = (start, stop) =>
      Math.hypot(stop.x - start.x, stop.y - start.y)
...
export const computeZIndex = ({ constants, props }) => () =>
  props.zIndex === constants.DEFAULT_POPPER_ZINDEX || props.zIndex < 1
