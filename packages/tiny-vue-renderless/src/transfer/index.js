...
export const getTargetData = ({ order, props, state, Tree, Table }) => () => {
  if (props.render && props.render.plugin.name === Tree) {
    ...
    props.modelValue.forEach((value) => {
      sortAry.push(
        rightData.filter((item) => item[props.props.key] === value)[0]
      )
    })
...
export const addToRight = ({ emit, refs, props, state, Tree }) => (value) => {
  ...
  if (props.render && props.render.plugin.name === Tree) {
    if (!props.treeOp.checkStrictly) {
      ...
      state.leftChecked.forEach(
        (item) => currentValue.indexOf(item) === -1 && currentValue.push(item)
      )
    }
  } else {
    if (value === 'all') {
...
export const logicFun = ({ props, emit, state }) => ({
  event,
  isAdd,
  pullMode
}) => {
  ...
  if (pullMode) {
    currentValue.splice(
      event.newIndex,
      0,
      currentValue.splice(event.oldIndex, 1)[0]
    )
  } else {
    const key = isAdd
    ...
    const index = isAdd
      ? state.rightChecked.indexOf(key)
      : state.leftChecked.indexOf(key)
    ...
    if (~index) {
      isAdd
        ? state.rightChecked.splice(index, 1)
        : state.leftChecked.splice(index, 1)
    }
...
export const getLeftCheckedData = ({ props, state }) => () =>
  state.sourceData.filter((item) => !item[props.props.disabled])

export const getRightCheckedData = ({ props, state }) => () =>
  state.targetData.filter((item) => !item[props.props.disabled])

export const recursiveData = ({ api, props, state }) => (
  data,
  isCasader,
  sign
) => {
  const propChildren =
    (props.treeOp && props.treeOp.props && props.treeOp.props.children) ||
    'children'
