...
export const computedShowHeader = ({ constants, props }) => () =>
  props.pattern !== constants.SIMPLE

export const computedShowFooter = ({ constants, props }) => () => {
  return (
    props.pattern === constants.LEGEND || props.pattern === constants.CLASSIC
  )
}

export const computedShowAside = ({ constants, props }) => () =>
  props.pattern !== constants.CLASSIC

const transferWidthOrHeight = (value) =>
...
export const computedAsideStyle = ({ constants, props }) => () => {
  if (
    props.pattern === constants.SIMPLE ||
    props.pattern === constants.FASHION
  ) {
    return {
...
export const computedMainStyle = ({ constants, props }) => () => {
  if (
    props.pattern === constants.DEFAULT ||
    props.pattern === constants.FASHION
  ) {
    return {
      top: transferWidthOrHeight(props.headerHeight),
