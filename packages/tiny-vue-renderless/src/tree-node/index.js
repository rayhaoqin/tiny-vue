...
export const watchIndeterminate = ({ api, props }) => (value) =>
  api.handleSelectChange(props.node.checked, value)

export const watchChecked = ({ api, props }) => (value) =>
...
export const getNodeKey = (state) => (node) =>
  getTreeNodeKey(state.tree.nodeKey, node.data)
...
export const handleSelectChange = ({ props, state }) => (
  checked,
  indeterminate
) => {
  ...
  if (
    ...
    state.oldIndeterminate !== indeterminate
  ) {
    const treeNodes = {}
...
export const handleClick = ({ api, vm, props, state, refs }) => (e) => {
  ...
  const isCheck = props.showCheckbox
    ? refs.checkbox && !refs.checkbox.$el.contains(e.target)
    : true
...
export const handleContextMenu = ({ treeRoot, vm, props, state, nextTick }) => (
  event
) => {
  ...
  nextTick(() => {
    ...
    if (event.clientY > -10) {
      state.menuposition.top = `${
        refs.node.offsetTop + refs.node.childNodes[0].offsetHeight / 2
      }px`
    }
    ...
    if (event.clientX > -10) {
      if (menuWidth > treeWidth) {
        ...
        state.menuposition.left =
          event.offsetX + menuWidth > treeWidth
            ? `${treeWidth - menuWidth}px`
            : `${event.offsetX}px`
      }
    }
  })
...
export const handleExpandIconClick = ({ api, state }) => (event, node) => {
  if (!node.isLeaf) {
    state.tree.iconTriggerClickNode
      ? api.handleExpandClick(true)
      : api.handleExpandClick(false)
    event.stopPropagation()
  }
}

export const handleExpandClick = ({ emit, vm, props, state }) => (
  nodeClickFlag
) => {
  if (nodeClickFlag) {
...
export const handleCheckChange = ({ nextTick, props, state }) => (
  value,
  event = {}
) => {
...
export const handleContentClick = ({ nextTick, props, state, emit }) => (
  currentNode,
  currentRadio
) => {
  if (props.showRadio) {
...
export const onSiblingToggleExpand = ({ state, props }) => ({
  isExpand,
  sibling
}) => {
  if (
    state.tree.accordion &&
    isExpand &&
    props.node &&
    sibling &&
    props.node.id !== sibling.id
  ) {
    props.node.collapse()
  }
}
