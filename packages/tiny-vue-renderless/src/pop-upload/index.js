...
export const computedCancel = ({ constants, props, t }) => () =>
  props.cancelButtonText || t(constants.CANCEL_BUTTTON_TEXT)

export const computedconfirm = ({ constants, props, t }) => () =>
...
export const computedUploadButtonText = ({ constants, props, t }) => () =>
...
export const beforeAvatarUpload = ({ props, state }) => (file) => {
  ...
  if (state.uploadFileType && state.uploadFileType.length != 0) {
    ...
    if (isnext) state.errorTips.push({ type: file.name })
  }
