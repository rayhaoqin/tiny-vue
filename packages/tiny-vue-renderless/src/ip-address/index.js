...
export const ipValidator = ({ props, api }) => (value) => {
  ...
  if (props.type) {
    ...
    if (api.isIP6(props.type)) {
      ...
      result = /^((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}$/.test(
        value
      )
    }
  }
...
export const keyup = ({ api, props }) => ({ item, index, event }) => {
  ...
  if (api.isIP4(props.type)) {
    ...
    if (
      (value === '0' || value > IPTHRESHOLD.NonNumeric || value.length === 3) &&
      !isNaN(event.key)
    ) {
...
const checkError1 = ({
  ...
  keyCode,
  value
}) => [Tab, Space, NumpadDecimal, NumpadComma].indexOf(keyCode) > -1 && value
