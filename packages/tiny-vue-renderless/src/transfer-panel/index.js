...
export const getFilterData = ({ api, props, state, Table, Tree }) => () => {
  if (state.renderType === Table) {
    ...
    if (props.filterMethod) {
      tableData = props.data.filter((item) =>
        props.filterMethod(state.query, item)
      )
    } else {
    ...
    if (props.showPager) {
      ...
      return tableData.slice(
        (state.internalPage - 1) * pageSize,
        pageSize * state.internalPage
      )
    } else {
      return tableData
    }
    ...
    return showFilterData(
      api.getFilterTreeData(copyArray(props.data)),
      'visible'
    )
  } else {
...
export const watchChecked = ({ api, emit, state }) => ({ value, oldvalue }) => {
  ...
  if (state.checkChangeByUser) {
    const movedKeys = value
      .concat(oldvalue)
      .filter((v) => value.indexOf(v) === -1 || oldvalue.indexOf(v) === -1)
...
export const watchData = ({ api, props, state, Tree }) => (value) => {
  ...
  if (state.renderType === Tree && !props.treeOp.checkStrictly) {
    ...
    const filteredDataKeys = state.filteredData.map(
      (item) => item[state.keyProp]
    )
...
export const watchDefaultCheckded = ({ api, props, state, Tree }) => ({
  value,
  oldvalue
}) => {
  if (
    ...
    value.every((item) => oldvalue.indexOf(item) > -1)
  ) {
    return
  }
  ...
  if (state.renderType === Tree && !props.treeOp.checkStrictly) {
    ...
    state.render.defaultCheckedKeys = state.checked = api.getStrictData(
      state.filteredData
    ).keys
  } else {
    const checked = []
    const checkableDataKeys = state.checkableData.map(
      (item) => item[state.keyProp]
    )
  ...
  if (state.renderType === Table || state.renderType === Tree) {
    ...
    if (state.renderType === Tree) {
      ...
      state.filteredData.forEach((data) => {
        state.checked.indexOf(data[state.keyProp]) > -1 &&
          pageCheckedKeys.push(data[state.keyProp])
      })
...
export const handleAllCheckedChange = ({ state, Table, Tree, vm }) => (
  value
) => {
  state.checked = value
    ? state.checkableData.map((item) => item[state.keyProp])
    : []
...
export const setPosition = ({ parent, state }) => (trend, event) => {
  ...
  const result =
    trend === 'up'
      ? [...checkedKey, ...uncheckedKey]
      : [...uncheckedKey, ...checkedKey]
