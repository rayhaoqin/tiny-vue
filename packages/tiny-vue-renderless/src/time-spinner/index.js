...
export const modifyDateField = ({ emit, props, state }) => (type, value) => {
  ...
  switch (type) {
    case 'hours':
      emit(
        'change',
        modifyTime(props.date, value, state.minutes, state.seconds)
      )
      break
    case 'minutes':
...
export const handleScroll = ({ api, refs }) => (type) => {
  const value = Math.min(
    Math.round(
      (refs[type].$refs.wrap.scrollTop -
        (api.scrollBarHeight(type) * 0.5 - 10) / api.typeItemHeight(type) +
        3) /
...
export const typeItemHeight = ({ refs }) => (type) =>
  refs[type].$el.querySelector(DATEPICKER.Qurtyli).offsetHeight
