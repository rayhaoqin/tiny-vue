...
const getDateArray = (str, dateFormat) => {
  ...
  for (let i = 0, len = str.length; i < len; i++) {
    ...
    const notNum = isNaN(charValue * 1) || charValue.trim() === ''
...
const innerParse = (value, dateFormat) => {
  if (typeof dateFormat === 'string') {
    ...
    const year = arr[0] * 1
    const month = arr[1] * 1
    const date = (arr[2] || 1) * 1
    const hours = (arr[3] || 0) * 1
    const minutes = (arr[4] || 0) * 1
    const seconds = (arr[5] || 0) * 1
    const milliseconds = (arr[6] || 0) * 1
