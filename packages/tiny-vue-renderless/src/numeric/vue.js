...
const initState = ({ reactive, computed, props, api, $service, parent }) => {
  const state = reactive({
    ...
    minDisabled: computed(
      () =>
        (!props.circulate && api.internalDecrease({ val: props.modelValue, step: props.step }) < props.min) ||
        state.formDisabled
    ),

    maxDisabled: computed(
      () =>
        (!props.circulate && api.internalIncrease({ val: props.modelValue, step: props.step }) > props.max) ||
        state.formDisabled
    ),
