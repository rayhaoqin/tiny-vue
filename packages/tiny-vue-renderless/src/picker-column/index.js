...
export const computedBaseOffset = ({ state, props }) => () => {
  return (state.itemHeight * (props.visibleItemCount - 1)) / 2
}
...
const isOptionDisabled = (option) => {
  return option !== null && typeof option === 'object' && option.disabled
}
...
export const onTouchmove = ({ state }) => (event) => {
  ...
  state.offset = range(
    state.startOffset + state.deltaY,
    -(state.count * state.itemHeight),
    state.itemHeight
  )
...
export const onTouchend = ({ api, state }) => () => {
  ...
  const allowMomentum =
    duration < state.momentumLimitTime &&
    Math.abs(distance) > state.momentumLimitDistance
  ...
  const index = range(
    Math.round(-state.offset / state.itemHeight),
    0,
    state.count - 1
  )
...
export const momentum = ({ api, state, props }) => (distance, duration) => {
  ...
  const index = range(
    Math.round(-distance / state.itemHeight),
    0,
    state.count - 1
  )
...
export const getValue = (state) => () =>
  state.columnsItem.values[state.currentIndex]
