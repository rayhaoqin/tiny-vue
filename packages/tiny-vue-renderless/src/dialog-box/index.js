...
export const computedAnimationName = ({ constants, props }) => () => {
  return props.rightSlide
    ? constants.DIALOG_SLIDER_RIGHT
    : constants.DIALOG_FADE
}

export const computedAddUnit = (value) =>
...
export const watchVisible = ({
  ...
  if (val) {
    ...
    props.rightSlide &&
      (el.querySelector(constants.DIALOG_BOX_CLASS).style.left = '')
  }
}
...
export const handleClose = ({ api, constants, emit, parent, props }) => (
  type = 'close'
) => {
  if (
...
const closeAllPopover = (parent) => {
  findPopoverComponent({ vm: parent, componentList: [] }).forEach(
    (component) => {
