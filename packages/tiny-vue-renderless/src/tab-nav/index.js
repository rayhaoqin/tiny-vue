...
} from '@opentiny/vue-renderless/common/deps/resize-event'
import { on, off } from '@opentiny/vue-renderless/common/deps/dom'

export const computedNavStyle = (state) => {
  const dir = ~[POSITION.Top, POSITION.Bottom].indexOf(state.rootTabs.position)
    ? 'X'
    : 'Y'
...
export const scrollIntoView = ({ parent, refs, state }) => () => {
  ...
  const to =
    activeTab.offsetLeft - (nav.offsetWidth - activeTab.offsetWidth) / 2
...
export const computedSizeName = (state) =>
  ~[POSITION.Top, POSITION.Bottom].indexOf(state.rootTabs.position)
    ? 'width'
    : 'height'
...
export const updated = ({ api, refs, state }) => () => {
  ...
  if (refs.tabBar) {
    refs.tabBar.state.barStyle = refs.tabBar.computedBarStyle(
      refs.tabBar,
      state
    )
  } else {
    ...
    line &&
      line.style &&
      (line.style.transform = api.computedNavStyle().transform)
  }
}
  ...
  if (state.mode === 'mobile') {
    if (activeTabBounding.left > navScrollBounding.width) {
      maxOffset =
        ...
        navScrollBounding.width +
        activeTabBounding.width
    } else {
