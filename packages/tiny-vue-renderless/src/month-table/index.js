...
import {
  ...
  arrayFind
} from '@opentiny/vue-renderless/date-table'
...
export const getRows = ({ props, state, vm }) => () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 4; j++) {
      ...
      cell.inRange =
        time >= getMonthTimestamp(minDate) && time <= getMonthTimestamp(maxDate)
      ...
      cell.selected = arrayFind(
        selectedDate,
        (selectDate) => selectDate.getTime() === cellDate.getTime()
      )
...
export const cellMatchesDate = (props) => (cell, date) => {
  ...
  return (
    props.date.getFullYear() === value.getFullYear() &&
...
export const getCellStyle = ({ api, props }) => (cell) => {
  ...
  const execDate =
    typeof props.value === 'object' ? props.value : toDate(props.value)
...
export const getMonthOfCell = (props) => (month) =>
  new Date(props.date.getFullYear(), month, 1)
