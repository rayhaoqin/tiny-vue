...
export const rowToVisible = ($table, row) => {
  $table.$nextTick(() => {
    ...
    if (tableBodyVnode) {
      ...
      const trEl = gridbodyEl.querySelector(
        `[${ATTR_NAME}="${getRowid($table, row)}"]`
      )
      ...
      if (trEl) {
        ...
        const trOffsetTop =
          trEl.offsetTop + (trEl.offsetParent ? trEl.offsetParent.offsetTop : 0)
        ...
        if ($table.scrollYLoad) {
          gridbodyEl.scrollTop =
            ($table.afterFullData.indexOf(row) - 1) *
            $table.scrollYStore.rowHeight
        }
      }
    }
  })
...
function setBodyLeft(body, td, $table, column, move) {
  ...
  const tdOffsetLeft =
    td.offsetLeft + (td.offsetParent ? td.offsetParent.offsetLeft : 0)
  ...
  if (from) {
    ...
    if (isLeftArrow && fixedDom) {
      ...
      const division = div
        ...
        : fixedDom.getBoundingClientRect().right
...
function getNodeOffset(el, container, rest) {
  if (el) {
    ...
    if (
      container && (el === container || el.offsetParent === container)
        ? 0
        : el.offsetParent
    ) {
 ...
 */
export const getOffsetPos = (el, container) =>
  getNodeOffset(el, container, { left: 0, top: 0 })
...
export const getRowNodes = (trList, cellNode, targetCellNode) => {
  ...
  for (
    let rowIndex = Math.min(startRowIndex, targetRowIndex),
      rowLen = Math.max(startRowIndex, targetRowIndex);
    rowIndex <= rowLen;
    rowIndex++
  ) {
    const cells = []
    ...
    for (
      let colIndex = Math.min(startColIndex, targetColIndex),
        colLen = Math.max(startColIndex, targetColIndex);
      colIndex <= colLen;
      colIndex++
    ) {
