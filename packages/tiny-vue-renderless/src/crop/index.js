...
export const computedCropImages = ({ constants, t }) => () =>
...
export const cropImage = ({ api, emit, props, state }) => () => {
  ...
  if (props.cropType.toLowerCase() === 'base64') {
    emit(
      'cropdata',
      api.getCroppedCanvas().toDataURL('image/jpeg', props.quality)
    )
...
export const getCroppedCanvas = (state) => () =>
...
export const move = (state) => (offsetX, offsetY) =>
  state.cropper.move(offsetX, offsetY)

export const reset = (state) => () => state.cropper.reset()

export const setCropBoxData = (state) => ({ left, top, width, height }) =>
  state.cropper.setCropBoxData({ left, top, width, height })

export const setData = (state) => (data) =>
  data !== null && typeof data === 'object' && state.cropper.setData(data)
...
export const scale = (state) => (scaleX, scaleY) =>
  state.cropper.scale(scaleX, scaleY)

export const scaleX = (state) => (x) => state.cropper.scaleX(x)

export const scaleY = (state) => (x) => state.cropper.scaleY(x)

export const getContainerData = (state) => () =>
  state.cropper.getContainerData()
...
export const setCanvasData = (state) => ({ width, height, left, top }) =>
...
export const setImage = ({ api, state, props }) => (e) => {
  ...
  const size = toFileSize(
    Number(props.maxSize.match(/[0-9]+/gi)[0]),
    'B',
    unit || 'B'
  )
  ...
  if (
    ...
    Number(file.size) > Number(size.split('B')[0])
  ) {
    return
  }
