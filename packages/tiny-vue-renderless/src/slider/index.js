...
export const bindKeyDown = ({ api, props, state }) => (event) => {
  ...
  switch (event.keyCode) {
    ...
    case KEY_CODE.PageUp:
      currentValue =
        state.activeValue + Math.ceil(state.rangeDiff / props.numPages)
      break
...
export const bindMouseDown = ({ api, constants, mode, emit, state }) => (
  event
) => {
  ...
  const isClickBar =
    hasClass(handleEl, constants.sliderCls(mode)) ||
    hasClass(handleEl, constants.rangeCls(mode))
  const isClickBtn = hasClass(handleEl, constants.buttonCls(mode))
  ...
  if (isClickBar) {
    ...
    if (state.isDouble) {
      if (
        Math.abs(currentValue - state.leftBtnValue) >
        Math.abs(state.rightBtnValue - currentValue)
      ) {
...
export const setTipStyle = ({
  ...
  const moveSize =
    ((state.activeValue - props.min) / state.rangeDiff) * state.sliderSize
  ...
  if (props.vertical) {
    tipStyle.top =
      ...
      constants.TIP_HEIGHT / 2 +
      constants.HALF_BAR_HEIGHT
    tipStyle.left = -tipEl.clientWidth / 2 + constants.HALF_BAR_HEIGHT
  } else {
    tipStyle.top =
      -constants.TIP_HEIGHT -
      constants.BUTTON_SIZE / 2 +
      constants.HALF_BAR_HEIGHT
...
export const setActiveButtonValue = ({ api, emit, props, state }) => (
  value
) => {
...
export const calculateValue = ({ props, state }) => (event) => {
  ...
  if (
    ...
    if (props.vertical) {
      currentValue =
        props.max -
        ((event.touches[0].pageY - offset.top) / state.sliderSize) *
          state.rangeDiff
    } else {
      currentValue =
        props.min +
        ((event.touches[0].pageX - offset.left) / state.sliderSize) *
          state.rangeDiff
    }
  } else {
    if (props.vertical) {
      currentValue =
        props.max -
        ((event.pageY - offset.top) / state.sliderSize) * state.rangeDiff
    } else {
      currentValue =
        props.min +
        ((event.pageX - offset.left) / state.sliderSize) * state.rangeDiff
    }
  }
...
export const getActiveButtonValue = (state) => () =>
  state.isDouble
    ? [state.leftBtnValue, state.rightBtnValue]
    : state.leftBtnValue
...
export const watchActiveValue = ({ api, emit, props, state }) => (
  newValue,
  oldValue
) => {
