...
export const minTimeOfDay = ({ MIN_TIME }) => (date) =>
  modifyDate(MIN_TIME, date.getFullYear(), date.getMonth(), date.getDate())

export const maxTimeOfDay = ({ MAX_TIME }) => (date) =>
...
export const watchValue = ({ api, state }) => (value) => {
  if (Array.isArray(value)) {
    if (value[0]) {
      ...
      state.minDate = value[1]
        ? new Date(api.minTimeOfDay(new Date(value[1])).getTime())
        : new Date()
    }
...
export const handleChange = ({ api, emit, refs, state }) => () => {
  if (api.isValidValue([state.minDate, state.maxDate])) {
    refs.minSpinner.state.selectableRange = [
      [api.minTimeOfDay(state.minDate), state.maxDate]
    ]
    refs.maxSpinner.state.selectableRange = [
      [state.minDate, api.maxTimeOfDay(state.maxDate)]
    ]
