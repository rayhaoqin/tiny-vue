...
export const computedTreeItemArray = () => (props, state) =>
  Array.prototype.slice.call(state.treeItems)

export const computedShowEmptyText = ({ constants, t }) => (props) =>
...
export const watchDefaultExpandedKeys = ({ state, api, nextTick }) => (
  value
) => {
...
export const dragOver = ({ state, emit, props, refs, vm }) => (
  event,
  dropNode
) => {
...
const setIsCurrent = (
  ...
  for (let i = 0; i < root.childNodes.length; i++) {
    ...
    if (
      child.data &&
      child.data[nodeKey] &&
      child.data[nodeKey] === defaultExpandedKeysHighlight
    ) {
...
export const getNodeKey = (props) => (node) =>
  innerGetNodekey(props.nodekey, node.data)
...
export const getCheckedKeys = (state) => (leafOnly) =>
  state.store.getCheckedKeys(leafOnly)
...
export const getHalfCheckedNodes = (state) => () => {
  return state.store.getHalfCheckedNodes()
}

export const getHalfCheckedKeys = (state) => () => {
...
export const getNode = (state) => (data) => {
...
export const handleKeydown = ({ vm, state }) => (event) => {
  ...
  if ([KEY_CODE.ArrowUp, KEY_CODE.ArrowDown].indexOf(keyCode) > -1) {
    ...
    if (keyCode === KEY_CODE.ArrowUp) {
      ...
      nextIndex =
        currentIndex < state.treeItemArray.length - 1 ? currentIndex + 1 : 0
    }
...
export const clearCurrentStore = (state) => (node) => {
  if (
    state.currentStore.flag &&
    state.currentStore.node &&
    state.currentStore.node.id !== node.id
  ) {
    doClearCurrentStore(state)
  }
}
