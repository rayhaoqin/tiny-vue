...
export const selectDay = (state) => (day) => {
  if (day && day.value) {
    ...
    state.selectedDate = +new Date(`${state.activeYear}/${state.activeMonth}/${day.value}`)
  }
}
  ...
  if (state.eventList && state.eventList.length) {
    events = copyArray(state.eventList)
      .filter(({ time }) => {
        ...
        return (
          date.getDate() === day &&
          date.getFullYear() === +state.activeYear &&
          date.getMonth() + 1 === +state.activeMonth
        )
      })
      .map((event) => {
 ...
 * @returns {Number}
 */
export const getTime = (state) => (day) => +new Date(`${state.activeYear}/${state.activeMonth}/${day}`)
