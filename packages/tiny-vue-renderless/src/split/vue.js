...
export const useOffset = ({ nextTick, props, refs, constants, hooks }) => {
  ...
  const state = reactive({
    ...
    offsetSize: computed(() =>
      state.isHorizontal ? 'offsetWidth' : 'offsetHeight'
    )
  })
...
export const renderless = (
  ...
  const state = reactive({
    ...
    computedleftTopMin: computed(() =>
      api.getComputedThresholdValue('leftTopMin')
    ),
    computedrightBottomMin: computed(() =>
      api.getComputedThresholdValue('rightBottomMin')
    ),
