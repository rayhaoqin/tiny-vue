...
export const initService = ({ props, service }) => {
  ...
  const httpRequestFn = noopFnCreator('httpRequest')
    ...
    rawFiles.forEach((rawFile) => {
      ...
      api.beforeUpload(file, true, (file) => {
        ...
        new Promise((resolve) => {
          ...
          if (props.autoUpload) {
            const tokenParams = { token: props.edm.upload.token, file: file, type: 'upload' }
...
const downloadFileSingle = (args) => {
  ...
  let sdResolver = (url) => {
    ...
    service
      ...
      .then((data) => {
        if (getKiaScanTip({ data, t, Modal, constants })) return
...
const downloadFileBatch = (args) => {
  ...
  let tokenParams = { token: downloadOps.packageToken, file: file, type: 'download' }
  api.getToken(tokenParams).then((data) => {
    if (!data) {
      return
    }
...
export const previewFile = ({ api, constants, props, service, state }) => (file) => {
  return new Promise((resolve, reject) => {
    try {
      const tokenParams = { isOnlinePreview: true, file: file, type: 'preview', token: props.edm.preview.token }
